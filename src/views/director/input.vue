        ...
        <Form ref="formValidate" :model="formValidate" :rules="ruleValidate" :label-width="120" class="form">
            <Row>
                ...
                <Form-item label="本月预排完成率" prop="prescheduling">
                    ...
                    <label>
                        <Input-number :max="999" :min="1" v-model="formValidate.prescheduling"></Input-number> %</label>
                </Form-item>
                </Col>
                <Col span="12">
                <Form-item label="本月结转完成率" prop="carryover">
                    <label>
                        <Input-number :max="999" :min="1" v-model="formValidate.carryover"></Input-number> %</label>
                </Form-item>
                </Col>
            </Row>
            <Row>
                <Col span="12">
                <Form-item label="Q1预排完成率" prop="Q1prescheduling">
                    <label>
                        <Input-number :max="999" :min="1" v-model="formValidate.Q1prescheduling"></Input-number> %</label>
                </Form-item>
                </Col>
                <Col span="12">
                <Form-item label="Q1结转完成率" prop="Q1carryover">
                    <label>
                        <Input-number :max="999" :min="1" v-model="formValidate.Q1carryover"></Input-number> %</label>
                </Form-item>
                </Col>
            </Row>
            ...
            <Form-item class="btngroup">
                ...
                <Button type="ghost" @click="handleReset('formValidate')" style="margin-left:60px;" >重置</Button> 
            </Form-item>
        </Form>
    </div>
</template>
...
export default {
    data() {
        return {
            ...
            ruleValidate: {
            prescheduling:[{
                required:true,message:'本月预排不能为空',trigger:'blur'
            }],// 本月预排
               carryover:[{
                required:true,message:'本月结转不能为空',trigger:'blur'
               }],// 本月结转
               Q1prescheduling:[{
                required:true,message:'Q1预排不能为空',trigger:'blur'
               }],// Q1预排
               Q1carryover:[{
                required:true,message:' Q1结转不能为空',trigger:'blur'
               }],// Q1结转
               takesteps:[{
                required:true,message:'采取措施和情况不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 采取措施和情况
               speed:[{
                required:true,message:'结转速度必选',trigger:'blur'
               }],// 结转速度
               reason:[{
                required:true,message:'结转原因不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 结转原因
               takemeasures:[{
                required:true,message:'结转采取措施不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 结转采取措施
               research:[{
                required:true,message:'教研情况不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 教研情况
               development:[{
                required:true,message:'研发情况不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 研发情况
               others:[{
                required:true,message:'其他职能工作情况不能为空',trigger:'blur'
               },{
                 max:500, message: '字数不能超过500个字', trigger: 'change'
               }],// 其他职能工作情况
               remarks:[{
                type: 'string', min: 0, max:200, message: '备注字数限制在0到200字', trigger: 'blur'
               }]// 备注
            }
        }
    },
