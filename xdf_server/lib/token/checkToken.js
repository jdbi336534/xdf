...
module.exports = async ( ctx, next ) => {
    ...
    const username=ctx.cookies.get('dotcom_user');
    // let decoded = jwt.decode(token,'sinner77');
    if ((!token)&&(!username)) {
        ctx.body={
            code:401,
    ...
    try {
        ...
        let tokenContent = await jwt.verify(token, 'sinner77');     
        // 格式为{ user_id: '123123123', iat: 1494405235, exp: 1494405235 }
        if(token && tokenContent.exp <= new Date()/1000){
            ctx.body={
            code:401,
            msg:'token过期，请重新登录！'
        }
